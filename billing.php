<?php
	...
	$sql = "SELECT CONCAT( app_entities.prefix, '-', app_entities.number ) AS 'Order ID', app_accounts.id AS 'Account ID', app_accounts.first_name AS 'First Name', app_accounts.last_name AS 'Last Name', app_accounts.company_name AS 'Company Name', app_order_header.dispatched AS 'Invoice Date', app_order_header.total_tariff AS 'Invoice Amount', app_payments.date_received AS 'Paid Date', app_payments.amount AS 'Amount Paid', (
`app_order_header`.`total_tariff` - `app_payments`.`amount` 
) AS 'Balance Due'
...
";
	...
	if ($result->num_rows > 0) {		
		...
		while($row = $result->fetch_assoc()) {
			...
			$qq="
			insert into customBilling (orderID,accountID,fname,lname,cname,invoicedate,invoiceAmount,PaidDate,AmountPaid,due) values
			(
				'".$row['Order ID']."',
				'".$row['Account ID']."',
				'".$row['First Name']."',
