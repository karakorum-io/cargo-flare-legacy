<?php
	...
	*/
	...
	class QuickDB
	{
		...
		public $scon			= null;		// for db slave connection
		...
		public function __construct($host, $user, $password, $db, $persistent = false, $show_errors = false, $emoji_support = false)
		{
			...
			if (SLAVE_DATABASE == 1)
			{
				$slave_selection = rand(1, SLAVE_NUMBER);
				
				$this->scon = @mysqli_connect(constant('DB_SERVER_SLAVE_'.$slave_selection), constant('DB_USERNAME_SLAVE_'.$slave_selection), constant('DB_PASSWORD_SLAVE_'.$slave_selection), constant('DB_NAME_SLAVE_'.$slave_selection));
			...
			if ($this->con)
			{
				if ($emoji_support)
				{
					...
					if (SLAVE_DATABASE == 1 AND $this->scon)
					{
						mysqli_query($this->scon, "SET NAMES utf8mb4");
						mysqli_query($this->scon, "SET CHARACTER SET utf8mb4");
						mysqli_query($this->scon, "SET COLLATION_CONNECTION = 'utf8mb4_unicode_ci'");
					}
				}
				else
				{
					...
					if (SLAVE_DATABASE == 1 AND $this->scon)
					{
						mysqli_query($this->scon, "SET NAMES utf8");
						mysqli_query($this->scon, "SET CHARACTER SET utf8");
						mysqli_query($this->scon, "SET COLLATION_CONNECTION = 'utf8_general_ci'");
					}
				}
		...
		public function execute($command)
		{
			...
			else
			{
				...
				if (SLAVE_DATABASE == 1)
				{
					$this->result = @mysqli_query($this->scon, $command) or $this->setError(mysqli_error($this->scon), mysqli_errno($this->scon));
				}
				else
				{
					$this->result = @mysqli_query($this->con, $command) or $this->setError(mysqli_error($this->con), mysqli_errno($this->con));
	...
	class QuickMSDB
	{
		...
		public function __construct($host, $user, $password, $db, $persistent = false, $show_errors = false, $emoji_support = false)
		{
			...
			if (SLAVE_DATABASE == 1)
			{
				...
				$db = constant('DB_NAME_SLAVE_'.$slave_selection);
				$user = constant('DB_USERNAME_SLAVE_'.$slave_selection);
				$password = constant('DB_PASSWORD_SLAVE_'.$slave_selection);
				
				$connectionInfo = array( "Database"=>$db, "UID"=>$user, "PWD"=>$password);
				
				$this->scon = @sqlsrv_connect(constant('DB_SERVER_SLAVE_'.$slave_selection), $connectionInfo);
		...
		{	
			...
			else
			{
				...
				if (SLAVE_DATABASE == 1)
				{
					$this->result = @sqlsrv_query($this->scon, $command, array(), array( "Scrollable" => 'static' ));
				}
				else
				{
					$this->result = @sqlsrv_query($this->con, $command, array(), array( "Scrollable" => 'static' ));
