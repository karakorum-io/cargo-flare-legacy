...
$str = '<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolor="#cdcdcd" style="font-size: 12px;">
			<tr bgcolor="#297eaf" >
				...
				<td width="8%" rowspan="2" align="right" style="padding:3px;"><b><center><font color="white">Deposit</font></center></b></td>
...
and APN.deleted = 0
AND (APN.`text` LIKE '%<green>CREDIT CARD ENDING WITH%'
...
AND AOH.parentid = 463
order by APN.created DESC";
...
if ($daffny->DB->num_rows() > 0) {
    ...
    while ($row = $daffny->DB->fetch_row($result)) {
		...
		$TAmount = "$".trim(explode("$", $row['TAmount'])[1]);
		...
		if( in_array($row['PaidBy'], [2,3,8,9]) ){
			...
			if(str_replace("$","",$TAmount) < $row['total_tariff']){
				$cPay = $row['total_tariff'] - str_replace("$","",$TAmount);
				$tmp_total_carrier_pay = $cPay;
			} else {
				// nothing to do
			}
		}
        ...
        $str .= '<tr>
			...
			<td align="right" bgcolor="' . $bgcolor . '" style="padding:3px;"><Strong>' .$TAmount . '</strong></td>
        ...
        $str .= '<td align="right" bgcolor="' . $bgcolor . '" style="padding:3px;">$ ' . number_format(($row['total_deposite']), 0) . '</td>
				<td align="right" bgcolor="' . $bgcolor . '" style="padding:3px;">$ ' . number_format(($row['total_tariff']), 0) . '</td>
        ...
        $TransactionAmount += trim(explode("$", $row['TAmount'])[1]);
    ...
    $str .= '<tr bgcolor="#297eaf" >
			...
			<td align="right"   style="padding:3px;"><font color="white"><b>$' . number_format($total_deposite, 2) . '</b></font></td>
...
ORDER BY dispatched DESC LIMIT 0 , 100 ";
...
<tr>
		 <td><div><img src="https://cargoflare.com/styles/cargo_flare/logo.png" alt="Cargo Flare" width="210" height="75"></div></td>
...
$str = '<br><table width="100%"  border="0" cellpadding="0" cellspacing="0" >
		<tr>
		 <td><div><img src="https://cargoflare.com/styles/cargo_flare/logo.png" alt="Cargo Flare" width="210" height="75"></div></td>
		 <td align="right"><h3>Run Date: ' . date("l jS \of F Y h:i:s A") . '</h3></td>
		</tr>
		</table>';
$str .= '<div><h2>Cancelled Orders Report ' . date("m-d-Y") . '</h2></div>';
$str .= '<table width="100%"  border="1" cellpadding="3" cellspacing="0" bordercolor="#000000" style="font-size: 12px;">';
$str .= '<tr bgcolor="#297eaf">';
$str .= '<td style="color:#fff;" align="center">Order ID</td>';
$str .= '<td style="color:#fff;" align="center">Order Date</td>';
$str .= '<td style="color:#fff;" align="center">Cancellation Date</td>';
$str .= '<td style="color:#fff;" align="center">Cancel Reason</td>';
$str .= '<td style="color:#fff;" align="center">Agent</td>';
$str .= '<td style="color:#fff;" align="center">Shipper</td>';
$str .= '<td style="color:#fff;" align="center">Tariff</td>';
$str .= '<td style="color:#fff;" align="center">Deposite</td>';
$str .= '<td style="color:#fff;" align="center">GP%</td>';
$str .= '</tr>';

// fetching data for cancellation report here
$sql = "SELECT * FROM app_order_header WHERE type=3 AND status=3 AND ( archived>='" . date('Y-m-d 00:00:00') . "' AND archived <='" . date('Y-m-d 23:59:00') . "' )";
$result = $daffny->DB->query($sql);
...
if ($daffny->DB->num_rows() > 0) {
    ...
    while ($row = $daffny->DB->fetch_row($result)) {
        if (($i % 2) == 0) {$bgcolor = "#ffffff";} else { $bgcolor = "#cfcfcf";}
        $shipper = $row['shipperfname'] . "" . $row['shipperlname'] . "<br>" . $row['shipperphone1'] . "<br>" . $row['shippercompany'];
        $gp = ((($row['total_tariff_stored'] - $row['carrier_pay_stored']) * 100) / $row['total_tariff_stored']);
        $totalTariff += $row['total_tariff_stored'];
        $totalDeposit += ($row['total_tariff_stored'] - $row['carrier_pay_stored']);
        $str .= '<tr>
				<td align="center" bgcolor="' . $bgcolor . '" style="padding:3px;background-color:' . $bgcolor . '">' . $row['prefix'] . "-" . $row['number'] . '</td>
				<td align="center" bgcolor="' . $bgcolor . '" style="padding:3px;background-color:' . $bgcolor . '">' . date('m-d-Y', strtotime($row['ordered'])) . '</td>
				<td align="center" bgcolor="' . $bgcolor . '" style="padding:3px;background-color:' . $bgcolor . '">' . date('m-d-Y', strtotime($row['archived'])) . '</td>
				<td align="center" bgcolor="' . $bgcolor . '" style="padding:3px;background-color:' . $bgcolor . '">' . $row['cancel_reason'] . '</td>
				<td align="center" bgcolor="' . $bgcolor . '" style="padding:3px;background-color:' . $bgcolor . '">' . $row['AssignedName'] . '</td>
				<td align="center" bgcolor="' . $bgcolor . '" style="padding:3px;background-color:' . $bgcolor . '">' . $shipper . '</td>
				<td align="right" bgcolor="' . $bgcolor . '" style="padding:3px;">$' . $row['total_tariff_stored'] . '</td>
				<td align="right" bgcolor="' . $bgcolor . '" style="padding:3px;">$' . ($row['total_tariff_stored'] - $row['carrier_pay_stored']) . '</td>
				<td align="right" bgcolor="' . $bgcolor . '" style="padding:3px;">' . number_format((float) $gp, 2, '.', '') . '%</td>
				</tr>';
        $i = $i + 1;
    }

    // total gp% calculation
    $totalGPPercent = (($totalDeposit * 100) / $totalTariff);
    $str .= '<tr><td colspan="6"></td><td align="right">$' . $totalTariff . '</td><td align="right">$' . $totalDeposit . '</td><td align="right">' . number_format((float) $totalGPPercent, 2, '.', '') . '%</td></tr>';
} else {
...
$str .= '</table>';
$sub = "Order Cancellation Report Till " . date("m-d-Y H:i:s");
$fname = "checkordercancellation_" . date("m-d-Y") . "_" . md5(mt_rand() . " " . time());
$cancellation_pdf_path = ROOT_PATH . "cronjobs/pdf/" . $fname;
getPdfNew("F", $cancellation_pdf_path, $str, $sub);
echo $str;

/**
 * Cancellation Report with reasons ends here
 */
...
try {
    ...
    $mail->AddAttachment($cancellation_pdf_path, "Order Cancellation Report on " . date("m-d-y") . ".pdf");
    //$mail->Send();
