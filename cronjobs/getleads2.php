...
include_once("gl_topautoleads.com.php");

include_once("gl_auto_transport.us.php");

include_once("gl_autotransportdepot.com.php");

include_once("gl_automobileshippingquotes.com.php");

include_once("gl_merakmoving.net.php");

include_once("gl_carriersoft.com.php");

include_once("gl_m3lgs.com.php");

include_once("gl_imoverleads.com.php");

include_once("gl_autotransport411.com.php");

include_once("gl_carshippingpro.net.php");

include_once("gl_irelocation.com.php");

include_once("gl_crowleyautotransport.com.php");

include_once("gl_mymovingquote.com.php");

include_once("gl_123movers.com.php");
...
foreach ($members as $m => $member) {
    ...
    //$member->username . "." . $companyabbr . "@" . $daffny->cfg['maildomain'];
    ...
    foreach ($leadsources as $l => $ls) {
        ...
        for ($e = 1; $e <= $qty_emails; $e++) {
            ...
            try {
                ...
                //$source_id = $lsm->getForCron($member->id, $from);
...
//                    'vehicles_run' => ($parsed['vehicle_run'] == 1 ? 1 : 2),
                ...
                $upd_arr = array(
                   ...
                   // 'assigned_id' => $Assigned_id,//$member->id,
...
*/
             ...
             /***********************Get Assigned *************/
				...
				$qCheck = $daffny->DB->selectRows("*", "app_defaultsettings_ass", "WHERE owner_id = '" . $member->id . "' and leadsource_id='" . $ls->id . "' limit 0,1");
					
		           if(!empty($qCheck)){
					   
					     $qCheckReinitiate = $daffny->DB->selectRows("*", "app_defaultsettings_ass", "WHERE owner_id = '" . $member->id . "' and leadsource_id='" . $ls->id . "' and status=0 order by ord limit 0,1");
		                ...
		                if(empty($qCheckReinitiate)){
									...
									$status =0;
									// print "member_id='".$row['member_id']."' and leadsource_id='" . $ls->id . "' <br><br>";
					...
					//print "WHERE owner_id = '" . $member->id . "' and leadsource_id='" . $ls->id . "' and status=0 order by ord limit 0,1<br>";
					$q = $daffny->DB->selectRows("*", "app_defaultsettings_ass", "WHERE owner_id = '" . $member->id . "' and leadsource_id='" . $ls->id . "' and status=0 order by ord limit 0,1");
		            ...
		            foreach ($q as $row) {
						...
						$BatchLeadSource = $row['batch'];
						 ...
						 //print $BatchLeadSource." Status ".$StatusLeadSource;
						  $rows = $daffny->DB->selectRows('*', " app_leadsources_queue ", "WHERE member_id='".$row['member_id']."' and leadsource_id='" . $ls->id . "' ");
						  if(!empty($rows))
						  {
							  foreach ($rows as $rowQueue) {
								...
								if (!$entity->duplicate) {
								  ...
								  $batch_count = $rowQueue['batch_count'];
									...
									$daffny->DB->update("app_defaultsettings_ass", $arrLeadSource,"member_id='".$row['member_id']."' and leadsource_id='" . $ls->id . "' ");
