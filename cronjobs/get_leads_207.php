...
foreach ($members as $m => $member) {
    ...
    foreach ($leadsources as $l => $ls) {
        ...
        $mailbox = imap_open($daffny->cfg['MAILSTRING'], 'rite_way_207_1@freightdragon.info', 'Chetu2018?#Freightdragon');
        ...
        for ($e = 1; $e <= $qty_emails; $e++) {
            try {
                ...
                $qCheck = $daffny->DB->selectRows(
                        ...
                        "WHERE owner_id = '" . $member->id . "' and leadsource_id='207' limit 0,1"
                );
                ...
                if (!empty($qCheck)) {                    
                    $qCheckReinitiate = $daffny->DB->selectRows(
                            "*",
                            "app_defaultsettings_ass", 
                            "WHERE owner_id = '" . $member->id . "' and leadsource_id='207' and status=0 order by ord limit 0,1"
                    );
                    ...
                    if (empty($qCheckReinitiate)) {                        
                        ...
                        $daffny->DB->update("app_leadsources_queue", $arr, "  leadsource_id='207' ");
                        $status = 0;
                        $arrLeadSource = array("status" => 0);
                        $daffny->DB->update("app_defaultsettings_ass", $arrLeadSource, "leadsource_id='207' ");
                    }
                } else {
                    $Assigned_id = $member->id;
                }
                ...
                if (!empty($q)) {
                    ...
                    foreach ($q as $row) {
                        ...
                        $rows = $daffny->DB->selectRows(
                                ...
                                "WHERE member_id='" . $row['member_id'] . "' and leadsource_id='207' "
                        );
                        ...
                        if (!empty($rows)) {
                            ...
                            foreach ($rows as $rowQueue) {
                                ...
                                if (!$entity->duplicate) {
                                    ...
                                    $arr = array("batch_count" => $batch_count);
                                    $daffny->DB->update("app_leadsources_queue", $arr, " member_id='" . $row['member_id'] . "' and leadsource_id='207' ");
                                } else {
                                ...
                                $daffny->DB->update(
                                        "app_defaultsettings_ass",
                                        $arrLeadSource,
                                        "member_id='" . $row['member_id'] . "' and leadsource_id='207' "
                                );
