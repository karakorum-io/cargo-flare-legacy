...
$where = "e.`type` = 3 AND e.`status` = 7 limit 500,800";
print  $where;
...
$i=1;
        foreach ($rows as $row) {
			...
			if ($entity->status == Entity::STATUS_ISSUES && $entity->isPaidOff() ) {//&& trim($entity->archived) == ''
		            $entity->setStatus(Entity::STATUS_DELIVERED);
					print "<br>".$i."--".$entity->id."--".$entity->number."--".$entity->status;
					print "<br>==============================<br>";
			         $i++;
			        fflush();
	            }
