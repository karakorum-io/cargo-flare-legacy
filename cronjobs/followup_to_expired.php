...
ini_set('max_input_time', 80000000);
 ...
 $where = "e.`type` = 1 AND e.`status` = ".Entity::STATUS_LFOLLOWUP." and date_format(now(),'%Y-%m-%d')>=date_format(DATE_ADD(created,INTERVAL 21 DAY),'%Y-%m-%d') ";
...
$i=1;
        ...
        foreach ($rows as $row) {
            ...
            $entity->load($row['id']);
			$entity->setStatus(Entity::STATUS_LEXPIRED);
