...
try {
			...
			$mail->isHTML();
			$mail->Body = "Dispatch Activity Report " . date("m-d-Y H:i:s");//$str;
			$mail->Subject = "Dispatch Activity Report " . date("m-d-Y H:i:s");
			$mail->AddAddress("Jeff@ritewayautotransport.com");
			$mail->AddCC("nkumar@agilesoftsolutions.com");
			$mail->SetFrom($daffny->cfg['info_email']);
			$mail->AddAttachment($path, "Dispatch Activity Report ".date("m-d-y").".pdf");
			$mail->Send();
 ...
 function getPdfNew($out = "D", $path = "DispatchSheet.pdf",$str) {
		...
		$pdf->SetSubject("Dispatch Activity Report " . date("m-d-Y H:i:s")); 
		$pdf->SetTitle("Dispatch Activity Report " . date("m-d-Y H:i:s"));
	...
	$header = '<div style="text-align: center; font-weight: bold;height:230px;"><span class="dis_heading">Dispatch Activity Report ' . date("m-d-Y H:i:s").'</span></div>';
