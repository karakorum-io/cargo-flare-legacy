...
class CpHome extends CpAction {
	...
	public function idx() {
		...
		$products = $this->daffny->DB->selectRows('id, name', "products", "WHERE `is_online` = 1 AND `is_delete` = 0");
		...
		foreach ($products as $prod) {
			...
			$from = "products p, orders o, `orders_details` od";
			$where = "WHERE WEEK(o.`register_date`) = WEEK('".$date."') AND p.`id` = od.`product_id` AND od.`order_id` = o.`id` AND o.`status` = 2 AND p.`id` = ".(int)$prod['id']." GROUP BY `day`";
			...
			$from = "products p, orders o, `orders_details` od";
			$where = "WHERE MONTH(o.`register_date`) = MONTH('".$date."') AND p.`id` = od.`product_id` AND od.`order_id` = o.`id` AND o.`status` = 2 AND p.`id` = ".(int)$prod['id'];
