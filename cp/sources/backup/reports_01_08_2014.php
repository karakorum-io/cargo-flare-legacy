...
class CpReports extends CpAction {
	...
	public function sales() {
		...
		$select = "o.`id`, o.`register_date`, o.`status`, IF(o.`status` = ".Orders::STATUS_PROCESSED.", o.`amount`, NULL) as amount, o.`first_name`, o.`last_name`, p.`name` as `product_name`, l.`users`";
		$from = Orders::TABLE . " o LEFT JOIN " . License::TABLE . " l ON l.`order_id` = o.`id`, " . Product::TABLE . " p, `orders_details` od";
	...
	public function licenses() {
		...
		$where
			=
			"WHERE l.`order_id` = o.`id` AND l.`owner_id` = m.`id` AND od.`order_id` = o.`id` AND od.`product_id` = p.`id` AND cp.`owner_id` = m.`id` AND ds.`owner_id` = m.`id` AND p.`type_id` != "
			. Product::TYPE_ADDITIONAL;
		$this->applyOrder();
