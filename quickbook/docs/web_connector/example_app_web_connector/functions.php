...
$txnXML='<AppliedToTxnAdd>
   ...
   if(checkCodCop($arrEnt['balance_paid_by'])==1) 
   {
	   $amount = $arr['total_deposite'];	
   }
   
   
   if($arrPayment['amount'] > $amount )
      $txnXML='<IsAutoApply>true</IsAutoApply>';
	...
	<QBXMLMsgsRq onError="continueOnError">
		<ReceivePaymentAddRq>
			<ReceivePaymentAdd>
				...
				<TxnDate>'.date('Y-m-d', strtotime($arrPayment['date_received'])).'</TxnDate>
