...
<table width="100%" cellpadding="0" cellspacing="0" border="0" class="grid">
    <tr class="grid-head">
        ...
        <td><?=$this->order->getTitle("usedfor", "Used for")?></td>
    ...
    <? if (count($this->data)>0){?>
	    ...
	    <tr class="grid-body<?=($i == 0 ? " first-row" : "")?>" id="row-<?=$data['id']?>">
	        ...
	        <td align="center" style="text-transform:capitalize;"><?=htmlspecialchars($data['usedfor'])?></td>
	        <td style="width: 16px;"><?=editIcon(getLink("formtemplates", "edit", "id", $data['id']))?></td>
	        <td style="width: 16px;"><?=previewIcon(getLink("formtemplates", "show", "id", $data['id']))?></td>
	        <? if ($data['is_system'] != 1){?>
		        <td style="width: 16px;" class="grid-body-right"><?=deleteIcon(getLink("formtemplates", "delete", "id", $data['id']), "row-".$data['id'])?></td>
	        <?}else{?>
