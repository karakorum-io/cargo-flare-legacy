...
<div id="search_forms">
	...
	<div style="background:#fff;border:1px solid #ebedf2;" class="mb-5" id="basic_search_mainbox">
		...
		<div id="basic_search_board" class="hide_show hide_new_basic_search">
					...
					<div class="row">
						...
						<div class="col-6">							
							<div class="new_form-group">
								...
								<table cellpadding="0" cellspacing="0" border="0" class="null-padding" style="width:350px;display:none;" id="b_origin_city_search_table">
									<tr>
										<td>City:</td>
									...
									<tr>
										...
										<td>Zip:</td>
										<td><input type="text" class="form-box-textfield zip" name="b_origin_zip" id="b_origin_zip" style="width:75px;"/></td>
						...
						<div class="col-6">							
							<div class="new_form-group">
								...
								<table cellpadding="0" cellspacing="0" border="0" class="null-padding" style="margon-top:-19px;width:350px;display:none;" id="b_destination_city_search_table">
									<tr>
										<td>City:</td>
										<td colspan="4"><input type="text" class="form-box-textfield geo-city" name="b_destination_city" id="b_destination_city" style="width:275px;"/></td>
									</tr>
									<tr style="height:20px;"></tr>
									<tr>
										<td>State:</td>
										...
										<td>Zip:</td>
										<td><input type="text" class="form-box-textfield zip" name="b_destination_zip" id="b_destination_zip" style="width:75px;"/></td>
       ...
       <!--  -->
				...
				<table class="table table-bordered">
					...
					<?php if (count($this->searches) == 0) : ?>
						<tr>
							<td colspan="5" align="center"><i>You have no saved searches.</i></td>
						</tr>
					<?php endif; ?>
					<?php foreach ($this->searches as $i => $search) : ?>
						<tr class="grid-body<?= ($i == 0 ? " " : "") ?>" id="row-<?= $search->id ?>">
							...
							<td>
								<?php if (!is_null($search->get('destination_city_search'))) : ?>
									<?= $search->get('destination_city') ?>, <?= $search->get('destination_state') ?>, <?= $search->get('destination_zip') ?>
								<?php else : $regions = "";
							...
							<td style="width: 16px;"><img class="action-icon" title="Load" src="<?= SITE_IN ?>images/icons/download.png" width="16" height="16" onclick="loadSearchForm(<?= $search->id ?>)"/></td>
							<td style="width: 16px;" class="grid-body-right"><?= deleteIcon(getLink("vehicles", "search", "delete", $search->id), "row-" . $search->id) ?></td>
						</tr>
					<?php endforeach; ?>
				</table>
				<?= formBoxEnd() ?>
...
<?php if (isset($this->results)) : ?>
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="grid">
        <tr class="grid-head">
            <td align="center" class="grid-head-left">Balance Paid By</td>
            ...
            <td align="center" class="grid-head-right"><span style="white-space: nowrap;">Ship On</span>/Posted</td>
        </tr>
        <?php if (count($this->results) == 0) : ?>
            <tr class="grid-body first-row">
                <td class="grid-body-right grid-body-left" colspan="8" align="center"><i>Your search have no
                        results.</i></td>
            </tr>
        <?php endif; ?>
        <?php foreach ($this->results as $i => $result) : ?>
            ...
            <tr class="grid-body<?= ($i == 0) ? " first-row" : "" ?><?= $highlight ?>">
                <td align="center" class="grid-body-left"><?= $result[0]['balance_paid_by'] ?></td>
                <td align="center"><?= $result[0]['payment'] ?></td>
                <td align="center">
                    <strong><?= strtoupper($result[0]['ship_via']) ?></strong><br/>
                    <span class="like-link" onclick="mapIt(<?= $result[0]['entity_id'] ?>);">Map it</span><br/>
                    <strong><?= $result[0]['distance'] ?> mi
                        ($<?= number_format($result[0]['price'] / $result[0]['distance'], 2) ?>/mi)</strong>
                </td>
                <td align="center"><?= $result[0]['origin_city'] ?>, <?= $result[0]['origin_state'] ?>
                    , <?= $result[0]['origin_country'] ?></td>
                <td align="center"><?= $result[0]['destination_city'] ?>, <?= $result[0]['destination_state'] ?>
                    , <?= $result[0]['destination_country'] ?></td>
                <td align="center">
                    <strong>Load ID: <?= $result[0]['load_id'] ?></strong><br/>
                    <?php foreach ($result as $vehicle) : ?>
                        <div>
                            <?= $vehicle['vehicle_year'] ?> <?= $vehicle['vehicle_make'] ?> <?= $vehicle['vehicle_model'] ?>
                            &nbsp;<?= imageLink($vehicle['vehicle_year'] . " " . $vehicle['vehicle_make'] . " " . $vehicle['vehicle_model'] . " " . $vehicle['vehicle_type']) ?>
                            <?php if ($vehicle['vehicle_inop'] == '1') { ?><span
                                style="color:#f00">&nbsp;Inop</span><? } ?>
                        </div>
                    <?php endforeach; ?>
                </td>
                <td align="center">
