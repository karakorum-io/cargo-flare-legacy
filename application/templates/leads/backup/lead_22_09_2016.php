...
<?php //endif; ?>
<div style="display:none" id="notes">notes Test</div>
<br/>
...
<table cellspacing="0" cellpadding="0" width="100%" class="control-bar">
	<tr>
        ...
        <td><?= functionButton('Bulk Mail', 'sendBulkMail()') ?></td>
        <?php }?>
       ...
       <?php if($_GET['leads'] != '' && $_GET['leads'] != 'onhold'){?>
        <td>
         ...
         <td><?= functionButton('Remove Hold', 'changeStatusLeads('.Entity::STATUS_LFOLLOWUP.')') ?></td>
         <?php }else{?>
   ...
   foreach($this->entities as $i => $entity) :
	...
	<td align="center" class="grid-body-left"  width="10%">
		  ...
		  $_GET['leads'] == 'unreadable' || $_GET['etype'] ==1){
					...
					?>
                           <a href="<?= SITE_IN ?>application/leads/showimported/id/<?= $entity['entityid'] ?>"><div class="order_id"><?= $entity['number'] ?></div></a>
                     <?php }else { 
					 ...
					 ?>
					      <a href="<?= SITE_IN ?>application/leads/<?= $urlDetail ?>/id/<?= $entity['entityid'] ?>"><div class="order_id"><?= $entity['number'] ?></div></a>
                     <?php } ?>
                <br/>
