			...
			<tr>
				...
				<div style="max-height:400px; width:490px; overflow: auto; border: 1px solid #ccc; background-color: #f1f1f1; padding: 8px;">
					<? if (!empty($this->daffny->tpl->attachments)){?>
						...
						<?foreach ($this->daffny->tpl->attachments as $key=>$f ){?>
							<tr>
								<td width="20" align="center"><input type="checkbox" id="attachments<?=$f['id']?>" name="attachments[<?=$f['id']?>]" value="<?=$f['id']?>" <?=$f['ch']?> /></td>
								<td><label for="attachments<?=$f['id']?>"><?=$f['name']?></label></td>
							</tr>
						<?}?>
						</table>
					<?}else{
...
<br />
	<div class="lead-info" style="width: 950px;">
		...
		<div>
		    ...
		    <div id="codes_orders" style="display:none;">
				<? include(TPL_PATH."settings/codes_orders.php")?>
			</div>
			<div id="codes_quotes" style="display:none;">
				<? include(TPL_PATH."settings/codes_quotes.php")?>
			</div>
		</div>
	</div>
<br /><br />
