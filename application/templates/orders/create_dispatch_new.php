...
<div id="dispatch_form" >
    ...
    <form id="save_order_form" action="<?= getLink('orders/dispatchnew/id/' . $this->entity->id.'/acc/'.get_var('acc')) ?>" method="post" enctype="multipart/form-data">
		...
		<input type="hidden" name="make_member" value="<?php print $this->createMember; ?>"/>
			...
			<div id="carrier_information" class="pt-3 pb-3" style="padding-left:20px;padding-right:20px;">
				...
				<div class="row">
					...
					<div class="col-4">					
						<div class="new_form-group">
							@carrier_address@
							<div id="suggestions-box-carrier" class="suggestions"></div>
						</div>
					</div>
					...
					<div class="col-4">
						<div class="new_form-group">
							<label for="carrier_address">US Dot:</label>
							<input tabindex="25" name="us_dot" type="text" maxlength="255" class="form-box-textfield form-control" id="us_dot">
						</div>
					</div>
				...
				<div class="row">
					...
					<div class="col-4">					
						<div class="new_form-group">
							<label for="carrier_address">Address (2):</label>
							<input tabindex="25" name="carrier_address2" type="text" maxlength="255" class="form-box-textfield form-control" id="carrier_address2">
						</div>
					</div>
			...
			<div id="carrier_information" class="pt-3 pb-3" style="padding-left:20px;padding-right:20px;">
				...
				<div class="row"> 
					...
					<div class="col-3">
						<div class="new_form-group_4 new_form_group_2_input">
							<div class="row">
								<div class="col-10" id="pickup_state_div">
									@pickup_state@
								</div>
								<div class="col-2">
									@pickup_zip@
...
<script type="text/javascript">
	...
	$(document).ready(()=>{
		...
		document.querySelector('#pickup_address1').addEventListener('keyup', (e) => {
            const text = e.currentTarget.value;
            clearTimeout(timer);
            timer = setTimeout(() => {
                autoComplete($("#pickup_address1").val().trim(), 'pickup');
            }, waitTime);
        });

		document.querySelector('#carrier_address').addEventListener('keyup', (e) => {
            const text = e.currentTarget.value;
            clearTimeout(timer);
            timer = setTimeout(() => {
                autoComplete($("#carrier_address").val().trim(), 'carrier');
            }, waitTime);
        });

		document.querySelector('#deliver_address1').addEventListener('keyup', (e) => {
            const text = e.currentTarget.value;
            clearTimeout(timer);
            timer = setTimeout(() => {
                autoComplete($("#deliver_address1").val().trim(), 'destination');
            }, waitTime);
        });

		$(document).click(()=>{
            $(".suggestions").html("").html("").html("");
        });
	});
		...
		if(address.trim() != ""){
			$.ajax({
				...
				success: function (response) {
					...
					if(type == 'carrier'){
						h = document.getElementById("suggestions-box-carrier");
						h.innerHTML = "";
						functionName = 'applyAddressCarrier';
						html += '<ul class="typeahead dropdown-menu" role="listbox" style="top: 36px; width:350px; left: 133px; display: block;">';
                        html += '<li><a href="javascript:void(0)" style="height:25px !important; padding-top:0px !important; font-size:10px;">Suggestions</a></li>';
					}
	...
	function applyAddressCarrier(address, city, state, zip) {
		$("#suggestions-box-carrier").html("");
		$("#carrier_address").val(address);
		$("#carrier_city").val(city);
		$("#carrier_state").val(state);
		$("#carrier_zip").val(zip);
		document.getElementById("suggestions-box-carrier").innerHTML = "";
	}

	function applyAddressDestination (address, city, state, zip) {
		$("#suggestions-box-destination").html("");
		$("#deliver_address1").val(address);
		$("#deliver_city").val(city);
		$("#deliver_state").val(state);
		$("#deliver_zip").val(zip);
