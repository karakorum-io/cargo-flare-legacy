...
?>
<!--Force Account Contract Update Popup-->
<style>
    #blurbackground{
        width:100%;
    ...
    #contractCustomerEmail{
        margin-bottom:20px;        
    }
    #connercialContractEmailField{
        width:100%;
        height:30px;
        border:1px solid #ccc;
        padding-left:10px;
...
<div id="forceContractUpdatePopup">
    <div id='forcePopupContainer'>
        <h3>A new version of the account contract was updated on <UpdateDate>Blah Blah Blah</UpdateDate></h3>
        <div id='contentDiv'><div id='contentInnerDiv'>This is the email content</div></div>
        <div id='contractDiv'><div id='commercialComtractPreview'></div></div>
        <div id='contractCustomerEmail'><input id='connercialContractEmailField' type="email" name='customerEmail'  placeholder='Customer Email Address'></div>
        <div id='sendContractButtonDiv'><input id='sendContractButton' type='button' value='Send' class=''></div>
    </div>
</div>
<!--Force Account Contract Update Popup Closed-->
...
<script>
    ...
    $(document).ready(function(){
        $.ajax({
			url: '<?= SITE_IN ?>application/ajax/accounts.php',
			type: 'POST',
			dataType: 'json',
			data: {
				account_id: '<?php echo $this->entity->account_id;?>',
				action: 'getAccountContract'
			},
			success: function(data) {                            
					if(data.commercial_terms_updated_at <= data.version  ){
						$("#B2BNEW").html("<a target='_blank' href='<?php echo $protocol.$_SERVER['HTTP_HOST'];?>/account/contractPreview/id/"+data.id+"'>\n\
						<img src='/images/icons/b2b.png'>\n\
						</a>");
					} else {
						var assignedDate;
						if (data.commercial_terms_updated_at === undefined) {
							data.commercial_terms_updated_at = "";
						} else {
							data.commercial_terms_updated_at = new Date(data.commercial_terms_updated_at);
							data.commercial_terms_updated_at = data.commercial_terms_updated_at.toLocaleString();                                    
						}
						$("#blurbackground").show();
						$("#forceContractUpdatePopup").show();
						$("UpdateDate").html("");
						$("#contentInnerDiv").html("");
						$("UpdateDate").html(data.commercial_terms_updated_at);
						$("#commercialComtractPreview").html(data.commercial_terms);
						$("#connercialContractEmailField").val(data.customerEmail);
						$("#contentInnerDiv").html(data.emailTemplate);
					}
			}
		});
    });
