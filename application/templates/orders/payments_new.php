...
<div class="row" style="padding-left: 16px;">
	<div class="col-6 col-sm-12">
		<div class="form-group" style="margin-bottom:15px;">
			<label>The payment will appear in Freight Dragon once it's processed by the gateway. </label>
					...
					<div id="table_gateway" style="display:none;">		
						<div class="row">
							...
							<div class="col-7">
                                <div class="new_form-group">
                                    <div class="new_form-group">
                                        <label for="cc_fname">Selected Cards:</label>
                                        <select name="available_cards" id="available_cards" class="form-box-textfield form-control"></select>
                                    </div>
								</div>
                                <div class="new_form-group">
									@cc_fname@
...
<script>
    ...
    $("#available_cards").change(function(){
        console.log("Fetch and apply card");
        $.ajax({
		    url: BASE_PATH + 'application/ajax/accounts.php',
		    data: {
		        action: "GetCard",
                CardId: $("#available_cards option:selected").attr("CardId")
		    },
		    type: 'POST',
		    dataType: 'json',
		    success: function (response) {
		        if (response.success == true) {
		            $("#cc_fname").val(response.data.FirstName);
                    $("#cc_lname").val(response.data.LastName);
                    $("#cc_type").val(response.data.Type);
                    $("#cc_number").val(response.data.Number);
                    $("#cc_cvv2").val(response.data.CVV);
                    $("#cc_month").val(response.data.ExpiryMonth);
                    $("#cc_year").val(response.data.ExpiryYear);
                    $("#cc_address").val(response.data.Address);
                    $("#cc_city").val(response.data.City);
                    $("#cc_state").val(response.data.State);
                    $("#cc_zip").val(response.data.Zipcode);
		        }
		    }
		});
    });
