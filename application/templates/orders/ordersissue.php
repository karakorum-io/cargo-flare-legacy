...
<table id="testData5" style="display:none; visibility:hidden;">
        <tr>
    ...
    <tr>
        <td width="100%">&nbsp;</td>
           ...
           <form id="issue_form" action="/application/orders/searchIssue" method="post">
               <select id="issue_type" name="issue_type" onchange="getElementById('issue_form').submit();" class="elementname form-box-combobox" elementname="select" style="width:150px;">
               <option  value="">Select One</option>
               <option value="1" <?php if($_POST['issue_type']==1)print "selected=selected";?>>By Customer</option>
               <option value="2" <?php if($_POST['issue_type']==2)print "selected=selected";?>>By Carrier</option>
               <option value="3" <?php if($_POST['issue_type']==3)print "selected=selected";?>>Partial payments</option></select>
           </form></td>
           <td><?= functionButton('Print', 'printSelectedOrderForm()') ?></td>
           <td>@form_templates@</td>
           <td>
					   
                       <?php print functionButton('Email', 'emailSelectedOrderFormNew()'); ?>
					   <?php //print functionButton('Email', 'emailSelectedOrderForm()'); ?></td>
        ...
        <?= functionButton('Restore', 'restoreOrders()','',' brn btn-sm btn_light_green') ?>
            <?php endif; ?>
        </td>
        
        <?php if ($_GET['orders'] == "dispatched") { ?>
            <td  valign="top">
			 <?php print  functionButtonDate(Entity::STATUS_PICKEDUP,'Picked Up Date', 'setPickedUpStatusAndDate',false,'pickup_button1','yy-mm-dd');?>
        ...
        <?php } elseif ($_GET['orders'] == "pickedup") { ?>
            <td  valign="top">
			<?php print  functionButtonDate(Entity::STATUS_DELIVERED,'Delivered Date', 'setPickedUpStatusAndDate',false,'delivered_button1','yy-mm-dd');?>
        ...
        <?php if ($_GET['orders'] != "archived") { ?>
            <td  valign="top"><?= functionButton('Cancel Checked Order', 'cancelOrders()') ?></td>
        <?php } ?>
    </tr>
</table>
        ...
        <tr id="order_tr_<?= $entity->id ?>" class="grid-body<?= ($i == 0 ? " first-row" : "") ?>">
            ...
            <td class="grid-body-right">
                <table cellspacing="0" cellpadding="0" border="0" width="100%" class="tariff-grid">
                    ...
                    <tr>
                        <td width="10"><img src="<?= SITE_IN ?>images/icons/circle-black.png" alt="Deposit    "
                                            title="Deposit" width="16" height="16"/></td>
                        <td style="white-space: nowrap;"><span class='black'><b><?= ("$ " . number_format((float)$isValue['totalPayValue'], 2, ".", ",")) ?></b></span></td>
                    </tr>
                    <?php }?>
                </table>
            </td>
...
<table cellspacing="0" cellpadding="0" width="100%" class="control-bar">
    <tr>
        ...
        <?php if ($this->status != Entity::STATUS_ARCHIVED) { ?>
            <!--td>
                <select class="form-box-combobox" id="company_members_bottom">
                    <?php foreach($this->company_members as $member) : ?>
                        <option value="<?= $member->id ?>"><?= $member->contactname ?></option>
                    <?php endforeach; ?>
                </select>
            </td-->
            <td><?php print functionButton('Reassign Order', 'reassignOrdersDialog()'); ?>
        ...
        <?php if ($_GET['orders'] != "archived") { ?>
            <td><?= functionButton('Cancel Checked Order', 'cancelOrders()') ?></td>
        <?php } ?>
        <?php if ($_GET['orders'] == "") { ?>
            <td><?= functionButton('Post to FB', 'postToFB()') ?></td>
        <?php } elseif ($_GET['orders'] == "dispatched") { ?>
            <td><?= functionButton('Picked Up', 'setPickedUpStatus()') ?></td>
        <?php } elseif ($_GET['orders'] == "pickedup") { ?>
            <td><?= functionButton('Delivered', 'setDeliveredStatus()') ?></td>
        <?php } elseif ($_GET['orders'] == 'posted') { ?>
            <td><?= functionButton('Unpost', 'unpostFromFB()') ?></td>
        <?php } ?>
    </tr>
</table>
@pager@