...
<?php
    ...
    if (isset($_GET['orders'])){
        ...
        if (in_array($_GET['orders'], array("notsigned","dispatched","pickedup","delivered","issues","archived")) 
	         ...
	         ||
			in_array($_GET['mtype'], array(Entity::STATUS_NOTSIGNED,Entity::STATUS_DISPATCHED,Entity::STATUS_PICKEDUP,Entity::STATUS_DELIVERED,Entity::STATUS_ISSUES,Entity::STATUS_ARCHIVED))
			){
...
<table cellspacing="0" cellpadding="0" width="100%" class="control-bar">
    ...
    <tr>
       ...
       <?php if($this->status == Entity::STATUS_DELIVERED || $this->status == Entity::STATUS_PICKEDUP || $this->status == Entity::STATUS_ISSUES){
    ...
    <tr class="grid-head">
       ...
       <?php 
	     if($_GET['orders']=="searchorders" &&
			!in_array($_GET['mtype'], array(Entity::STATUS_ACTIVE,Entity::STATUS_POSTED,Entity::STATUS_NOTSIGNED,Entity::STATUS_DISPATCHED,Entity::STATUS_PICKEDUP,Entity::STATUS_DELIVERED,Entity::STATUS_ISSUES,Entity::STATUS_ARCHIVED))  ){
		 ?>
        ...
        <?php if ($avail_title == Entity::TITLE_PICKUP_DELIVERY):?>
           ...
           <td  width="7%">
              ...
              <?php print $this->order->getTitle("load_date", $avail_title) ?><? //print $avail_title; ?>
        ...
        <?php else : ?>
            ...
            <td  width="7%"><?= $avail_title; ?><?php //print $this->order->getTitle("avail", $avail_title) ?>
...
$totalTariff = 0;
	...
	flush();
        ...
        if (($_GET['orders'] == "searchorders" && $_GET['mtype'] == Entity::STATUS_ISSUES) || ($_GET['orders'] == "issues")){ 
				 ...
				 if ( isset($files) && count($files) ) { 
				               ...
				               foreach ($files as $file) { 
									...
									if ($pos === false) {}
										...
										<a <?=strtolower($file['type'])=='pdf'?"target=\"_blank\"":""?> href="<?=getLink("orders", "getdocs", "id", $file['id'])?>"><span style="font-weight: bold;" class="hint--bottom hint--rounded hint--bounce hint--success" data-hint="B2B Generated"><img src="<?= SITE_IN ?>images/icons/b2b.png" /></span></a>
...
?>
            <td bgcolor="<?= $bgcolor ?>"  width="16%">
			   ...
			   <?php }?>
                ...
                <?php if($entity['shipperphone2']!=""){$phone2 = str_replace($words, $wordsReplace, $entity['shipperphone2']);?><div class="shipper_number">
                   <?php if($mobileDevice==1){?>
...
<?php if (($_GET['orders'] == "issues") ||($_GET['orders'] == "searchorders" && $_GET['mtype'] == Entity::STATUS_ISSUES)){ ?>
<table cellspacing="0" cellpadding="0" width="100%" class="control-bar">
 <tr>
