    ...
    try {
        switch ($_GET['action']) {
            ...
            case "getByZipLocal":
                ...
                $row = $daffny->DB->selectRows("`State`, `City`", "`fd_zipcode_database`", "WHERE `Zipcode` LIKE '" . mysqli_real_escape_string($daffny->DB->connection_id, $_POST['zip']) . "'");
                ...
                if (!empty($row)) {
                    ...
                    $dataArr['status'] = "ok";
                    $dataSize = sizeof($row);
                    $dataArr['size'] = $dataSize;
                    if ($dataSize > 1) {

                        foreach ($row as $rows) {
                            $TempArr = array();
                            $TempArr['state'] = $rows['State'];
                            $TempArr['city'] = $rows['City'];
                            $dataArr['data'][] = $TempArr;
                        }
                    } else {

                        $TempArr = array();
                        $TempArr['state'] = $row[0]['State'];
                        $TempArr['city'] = $row[0]['City'];
                        $dataArr['data'][] = $TempArr;
                    }
