...
abstract class FdObject {
	 ...
	 */
        ...
        public function queueLoader($id = null) {
                //echo "ID-Queue-Loader".$id."<br>";
                //die('XXX');
		...
		$row = $this->db->selectRow("*", "quickbooks_queue", "WHERE `quickbooks_queue_id` = " .$id);
	...
	/**
	 * Load Object data from customer review feedback
	 * Throws exception if DB helper not set, invalid ID or ID not found in DB
	 * author chetu
	 */
        
        public function reviewLoader($id = null) {
                
		if (is_null($this->db)) throw new FDException(get_class($this) . "->load: DB helper not set");
		if (!ctype_digit((string)$id)) throw new FDException(get_class($this) . "->load: invalid ID");
		$row = $this->db->selectRow("*", "app_reviews", "WHERE `id` = " .$id);
	...
	public function print_receipt_loader($id = null){
		$row = $this->db->selectRow("*", "app_payments_check", "WHERE `id` = " .$id);
		foreach ($row as $key => $val) {
			$this->attributes[$key] = $val;
		}
		$this->loaded = true;
