...
class AccountManager extends FdObjectManager
{
    ...
    public function getActive($where = '')
    {
        ...
        $tables = " `app_accounts` ";
        $sql = "SELECT count(status) as number , status FROM {$tables} {$where} group by status";
    ...
    {
        $tables = " app_accounts AS A LEFT OUTER JOIN app_commision AS B ON A.id=B.shipper_id LEFT OUTER JOIN members M ON M.id = B.members_id";
        $sql = "SELECT count(A.status) as number , A.status FROM {$tables} WHERE {$where} group by A.status";
    ...
    {
        $tables = "app_accounts AS A INNER JOIN members M ON M.id = A.owner_id";
        $where_s = $where . " ";
        ...
        $sql = "SELECT M.contactname as contactname,A.id as ShipperID FROM {$tables} WHERE {$where_s} ";
        $result = $this->db->query($sql);
    ...
    public function getAccount($order = null, $per_page = 100, $where = '', $pagerWhere = '')
    {
        ...
        $tables = "`app_entities` A,  `app_accounts` B, `members` C ";
        $where_s = " A.Account_id = B.ID and A.Assigned_id = C.id  " . $where . " GROUP BY B.ID, B.Company_name, B.first_name,B.last_name, B.city,B.state, A.Assigned_id, C.contactname ";
        ...
        $sql = "SELECT B.id as ShipperID, B.Company_name as shipperCompany,B.first_name,B.last_name, B.city,B.state,  A.Assigned_id as AssignedID, C.contactname AssignedName,  COUNT( * ) AS Orders FROM {$tables} WHERE {$where_s} ";
    ...
    public function searchAccount($search_string, $ownerId, $type = null)
    {
        ...
        $rows = parent::get(null, null, $owner_id . " {$add_where}  AND (`company_name` LIKE('%{$search_string}%') OR `first_name` LIKE('%{$search_string}%') OR `last_name` LIKE('%{$search_string}') OR `phone1` LIKE('{$search_string}') OR `phone2` LIKE('{$search_string}') OR `contact_name1` LIKE('{$search_string}') OR `contact_name2` LIKE('{$search_string}'))");
        $accounts = array();
        foreach ($rows as $row) {
            $account = new Account($this->db);
            $account->load($row['id']);
    ...
    public function searchCarrier($search_string, $filter ,$memberId ,$ownerId)
    {
        if (!ctype_digit((string) $ownerId)) {
            throw new FDException("Invalid onwer ID");
        }
        ...
        $search_string = mysqli_real_escape_string($this->db->connection_id, $search_string);
        //$rows = parent::get(null, null, $owner_id . " {$add_where}  AND (`company_name` LIKE('%{$search_string}%') OR `first_name` LIKE('%{$search_string}%') OR `last_name` LIKE('%{$search_string}') OR `phone1` LIKE('{$search_string}') OR `phone2` LIKE('{$search_string}') OR `contact_name1` LIKE('{$search_string}') OR `insurance_iccmcnumber` LIKE('{$search_string}') OR `contact_name2` LIKE('{$search_string}'))");
        $rows = parent::get(null, null, $owner_id . " {$add_where}  AND `".$filter."` LIKE '%".$search_string."%' ");
        ...
        // fetching others
        $add_where = "AND `is_carrier` = 1 ";
        $owner_id = " `owner_id` != " .$ownerId." AND `member_id` IS NOT NULL ";
        $search_string = mysqli_real_escape_string($this->db->connection_id, $search_string);
        //$rows = parent::get(null, null, $owner_id . " {$add_where}  AND (`company_name` LIKE('%{$search_string}%') OR `first_name` LIKE('%{$search_string}%') OR `last_name` LIKE('%{$search_string}') OR `phone1` LIKE('{$search_string}') OR `phone2` LIKE('{$search_string}') OR `contact_name1` LIKE('{$search_string}') OR `insurance_iccmcnumber` LIKE('{$search_string}') OR `contact_name2` LIKE('{$search_string}'))");
        ...
        foreach ($rows as $row) {
            $account = new Account($this->db);
            $account->load($row['id']);
            $otherCarriers[] = $account;
        }

        return [
            'myCarriers' => $myCarriers,
            'otherCarriers' => $otherCarriers,
        ];

    }

    public function searchMyCarrier($search_string, $memberId ,$ownerId)
    {
        if (!ctype_digit((string) $ownerId)) {
            throw new FDException("Invalid onwer ID");
        }
        ...
        $owner_id = " `owner_id` = " .$ownerId." ";
        $search_string = mysqli_real_escape_string($this->db->connection_id, $search_string);
        $rows = parent::get(null, null, $owner_id . " {$add_where}  AND (`company_name` LIKE('%{$search_string}%') OR `first_name` LIKE('%{$search_string}%') OR `last_name` LIKE('%{$search_string}') OR `phone1` LIKE('{$search_string}') OR `phone2` LIKE('{$search_string}') OR `contact_name1` LIKE('{$search_string}') OR `contact_name2` LIKE('{$search_string}' OR `us_dot` LIKE('{$search_string}'))");

        return $rows;
    }

    public function searchAccountShipper($search_string, $ownerId, $type = null)
    {
        $orderTrigger = false;
        $orderId = $search_string;
        $orderId = explode("-", $orderId);

        $length = strlen($orderId[0]);
        if ($length == 3 && count($orderId) == 2) {
            $orderTrigger = true;
            $tempData = $this->db->select(" account_id", 'app_order_header', " where prefix = '" . $orderId[0] . "' AND number = '" . $orderId[1] . "' ");
            $dataTemp = mysqli_fetch_assoc($tempData);
            $search_string = (int) $dataTemp['account_id'];
        }
        ...
        if (!is_null($type)) {
            switch ($type) {
                case Account::TYPE_CARRIER:
                    $add_where = "AND `is_carrier` = 1";
                    $owner_id = "`owner_id` !='' ";
                    $this->searchCarreirLocation();
                    break;
                case Account::TYPE_SHIPPER:
                    $add_where = "AND `is_shipper` = 1";
                    $owner_id = "`owner_id` IN (" . implode(',', Member::getCompanyMembers($this->db, $_SESSION['member']['parent_id'])) . ")";
                    $o_id = implode(', ', Member::getCompanyMembers($this->db, $_SESSION['member']['parent_id']));
                    $parent_id = $_SESSION['member']['parent_id'];
                    break;
                case Account::TYPE_TERMINAL:
                    $this->searchCarreirLocation();
                    $add_where = "AND `is_location` = 1";
        ...
        if (Account::TYPE_SHIPPER) {
            if ($orderTrigger) {
                $dbs = $this->db->hardQuery('CALL searchShipperByOrderId(' . $search_string . ', "' . $parent_id . '")');
            } else {
                $dbs = $this->db->hardQuery('CALL shipper_popup("' . $search_string . '", "' . $parent_id . '")');
            }

            while ($row = mysqli_fetch_assoc($dbs)) {
                $resultSet['shipperData'][] = $row;
            }
            mysqli_free_result($dbs);

            mysqli_next_result($this->db->connection_id);
            $result2 = mysqli_use_result($this->db->connection_id);
            while ($row = mysqli_fetch_assoc($result2)) {
                $resultSet['shipperOrderData'][] = $row;
            }
            mysqli_free_result($result2);

            mysqli_next_result($this->db->connection_id);
            $result3 = mysqli_use_result($this->db->connection_id);
            while ($row = mysqli_fetch_assoc($result3)) {
                $resultSet['shipperLeadsData'][] = $row;
            }
            ...
            " . $shipperId . " OR `first_name` LIKE('%{$search_string}%')
            OR `last_name` LIKE('%{$search_string}')
            OR  CONCAT( `first_name`,  ' ', `last_name` ) LIKE('%{$search_string}%')
            OR `phone1` LIKE('{$search_string}') OR `cell` LIKE('{$search_string}') OR `fax` LIKE('{$search_string}')
            OR `phone2` LIKE('{$search_string}') OR `address1` LIKE('%" . $search_string . "%') OR `address2` LIKE('%" . $search_string . "%')
            OR `contact_name1` LIKE('{$search_string}') OR `city` LIKE('{$search_string}') OR `zip_code` LIKE('{$search_string}')
            OR `contact_name2` LIKE('{$search_string}')
            OR  CONCAT( `address1`,  ' ', `address2`, ' ', `city`, ' ', `state`, ' ', `zip_code` ) LIKE('%{$search_string}%')
            )");
            $accounts = array();
            foreach ($rows as $row) {
                $account = new Account($this->db);
    ...
    public function getShippersReports($order = "", $per_page, $cust_name, $report_arr = array(), $parent_id)
    {
        ...
        if (!is_null($per_page) && ctype_digit((string) $per_page)) {
            ...
            $where_s .= " ORder by " . $order . " " . $sort . " " . $this->pager->getLimit() . " ";
        }
        ...
        $sql = "Select A.account_id, A.created, A.assigned_id, B.first_name as shipperfname, B.last_name as shipperlname, B.company_name as shippercompany, B.email as shipperemail, B.phone1 as shipperphone1, B.phone1_ext as shipperphone1_ext, B.phone2 as shipperphone2, B.phone2_ext as shipperphone2_ext, case when A.source_name is null then A.referred_by else A.source_name end as `source_name`, A.AssignedName, count( * ) AS orders, sum( A.`total_tariff` ) AS tariff, sum( A.`total_carrier_pay` ) AS carrier, sum( A.`total_tariff` - `total_carrier_pay`) AS deposit, sum(CASE WHEN A.`FlagTarrif` = 2 THEN 1 ELSE 0 END ) AS invoices, sum(CASE WHEN A.`FlagTarrif` !=2 THEN 1 ELSE 0 END ) AS payments FROM app_order_header A inner join app_accounts B on A.account_id = B.id" . $where_s . "   ";
    ...
    public function getShippersReportsExport($order = "", $per_page, $cust_name, $report_arr = array(), $parent_id)
    {
        ...
        $where_x = "  GROUP BY A.account_id, A.assigned_id, B.first_name, B.last_name, B.company_name, B.email, B.phone1, B.phone1_ext, B.phone2, B.phone2_ext, case when A.source_name is null then A.referred_by else A.source_name end, A.AssignedName ORDER BY " . $_GET['order'] . " " . $_GET['arrow'] . "";
        $where_s = $where . $where_x;
    ...
    public function getCarriersReports($order = "", $per_page, $cust_name, $parent_id)
    {
        ...
        $inner_query = "SELECT COUNT( * ) , carrier_id FROM  `app_order_header` WHERE type =3 AND parentid =" . $parent_id . " AND status <> 3";
    ...
    public function getDuplicateAccount($order = null, $per_page = 100, $where = '', $pagerWhere = '', $accType = 0)
    {
        ...
        $tables = "`app_accounts` A, members B ";
    ...
    public function getDuplicateAccountDetails($order = null, $per_page = 100, $where = '', $pagerWhere = '', $acctype = 0)
    {
        ...
        $where_s = $where . " and  B.parent_id  = " . getParentId() . " group by A.`id` , A.`owner_id` , A.`company_name` " . $select . " , A.`phone1` , A.`phone2` , A.`phone1_ext` , A.`phone2_ext` , A.Address1, A.address2, A.`city` , A.`state` , A.`zip_code`";
    ...
    public function getShipperPaymentHistoryReports($order = "", $per_page, $cust_name, $report_arr = array(), $parent_id)
    {
        ...
        $where = " WHERE A.`parentid` = " . $parent_id . " AND A.STATUS =9 AND A.`type` =3 AND A.account_id <>0 AND `created` >= '" . $report_arr["start_date"] . "' AND `created` <= '" . $report_arr["end_date"] . "' group by A.`account_id`,
                        A.`shippercompany`, A.`shipperfname`, A.`shipperlname` order by orderCount desc";
        ...
        if (!is_null($per_page) && ctype_digit((string) $per_page)) {
            ...
            $this->pager->UrlStart = getLink();
            $this->pager->RecordsOnPage = $per_page;
            $this->pager->init($table, "X.orderCount", " WHERE A.`parentid` = " . $parent_id . " AND A.STATUS =9 AND A.`type` =3 AND A.account_id <>0 AND `created` >= '" . $report_arr["start_date"] . "' AND `created` <= '" . $report_arr["end_date"] . "' GROUP BY A.`account_id` , A.`shippercompany` , A.`shipperfname` , A.`shipperlname` ORDER BY orderCount DESC ) as X");
            $where_s .= " " . $this->pager->getLimit() . " ";
        }
    ...
    public function searchDuplicateAccounts($order = null, $per_page = 100, $where = '', $acctype = 0, $search_type, $value)
    {
        ...
        $sql = "SELECT A.id as ID, COUNT( C.id ) as number_of_count FROM {$tables} WHERE {$where_s} ";
