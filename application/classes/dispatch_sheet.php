...
class DispatchSheet extends FdObject
{
    ...
    public function reject()
    {
        ...
        $entity->update(array('carrier_id' => 0));
    ...
    public function cancel()
    {
        $entity = new Entity($this->db);
        $entity->load($this->entity_id);
        if ($entity->status == Entity::STATUS_DELIVERED) {
            throw new FDException("Can't cancel picked up Dispatch");
        }

        /* this is being done in the quickbooks response now*/
        $entity->setStatus(Entity::STATUS_ACTIVE);
        $this->update(array(
    ...
    public function getDispatchESignData() {
        $entity = new Entity($this->db);
        $entity->load($this->entity_id);
        ...
        $values = array(
            ...
            'vehicles' => $this->getVehiclesHtml(),
            'origin' => $entity->getOrigin()->city.", ".$entity->getOrigin()->state.", ".$entity->getOrigin()->zip,
            'destination' => $entity->getDestination()->city.", ".$entity->getDestination()->state.", ".$entity->getDestination()->zip
        );
    ...
    public function getHtml($tpl, $type = 'html', $showSignature = true)
    {
        if (!($tpl instanceof template)) {
            throw new FDException("Invalid template object");
        }
    ...
    public function getHtmlNew($tpl, $type = 'html', $showSignature = true)
    {
        ...
        $values = array(
            ...
            'payments_terms' => $this->payments_terms,
        ...
        if ($type == 'pdf') {
            $values['vehicles'] = $this->getVehiclesHtmlNew();
    ...
    public function getVehiclesHtml()
    {
        ...
        $vehicles_html = "";
        if (count($vehicles) > 0) {
            $vehicles_html = "<table class='table table-bordered table-striped'>";
    ...
    public function acceptNew()
    {
		...
		$body = "Thank you <strong>$this->sign_by</strong> for accepting <strong>$this->c_companyname's </strong> Dispatch Sheet on behalf of <strong>$this->carrier_company_name</strong>. For your convenience we have attached a copy of your dispatch sheet. If this was sent to you by error please contact <strong>$this->c_companyname</strong> at <strong>$this->c_dispatch_phone</strong>";
		$body .= "<br><br>Sincerely,<br>".$entity->getAssigned()->getCompanyProfile()->dispatch_contact;
		$body .= "<br>".$entity->getAssigned()->getCompanyProfile()->dispatch_email;
		$body .= "<br>".$entity->getAssigned()->getCompanyProfile()->dispatch_phone;
    ...
    public function getPdfNew($out = "D", $path = "DispatchSheet.pdf")
    {
        ...
        $header = '<table cellpadding="0" cellspacing="0" border="0" class="dispatch_table" width="100%" style="border:1px solid #000000;">
			<tr>
				<td class="group"  align="center">
                    <span >
                        <span class="dis_heading">TRANSPORT DISPATCH SHEET -- NOT TO BE USED AS A</span>
                        <span class="dis_heading_black"> BOL</span>
                    </span>
				</td>
			</tr>
			<tr>
        ...
        $pdf->SetHTMLHeader('<div style="text-align: center; font-weight: bold;border-left:1px solid #f26e21;border-right:1px solid #f26e21;border-top:1px solid #f26e21;height:230px;"><span class="dis_heading">TRANSPORT DISPATCH SHEET -- NOT TO BE USED AS A BOL</span><br><span class="dis_heading_small1">SUBJECT TO THE TERMS AND CONDITIONS--QUESTIONS?</span> <span class="dis_heading_vsmall">' . $this->c_dispatch_phone . '</span></div>', 'O');
        $pdf->SetHTMLHeader('<div style="text-align: center; font-weight: bold;border-left:1px solid #f26e21;border-right:1px solid #f26e21;border-top:1px solid #f26e21;height:230px;"><span class="dis_heading">TRANSPORT DISPATCH SHEET -- NOT TO BE USED AS A BOL</span><br><span class="dis_heading_small1">SUBJECT TO THE TERMS AND CONDITIONS--QUESTIONS?</span> <span class="dis_heading_vsmall">' . $this->c_dispatch_phone . '</span></div>', 'E');

        $footer = ' <table cellpadding="11" cellspacing="11"   width="800" height="400"   style="background-color:#ffffff; border:1px solid #f26e21;">
			<tr>
				<td>
			...
			<tr>
				<td>
					<span class="dis_heading_small">Powered by CargoFlare</span>
				</td>
				<td>
        ...
        $html = $this->getHtmlNew(new template(), 'pdf');
        $html = str_replace("FreightDragon.com","CargoFlare.com",$html);
