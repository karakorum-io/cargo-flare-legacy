...
class Entity extends FdObject
{
    ...
    const CENTRAL_DISPATCH_EMAIL_TO = "cdupd-v4@api.centraldispatch.com";
    ...
    {
        if (strtotime($this->avail_pickup_date) == 0) {
            return "";
        }
    ...
    {
        if (strtotime($this->est_ship_date) == 0) {
            return "";
        }
    ...
    public function setStatus($status = null)
    {
        ...
        if ($status == Entity::STATUS_DISPATCHED) {
            ...
            if($this->dispatched == ""){
                $this->update(array("dispatched" => $current_time,"original_dispatched" => $current_time,'is_dispatched' => 1));
            } else {
                $this->update(array("dispatched" => $current_time,'is_dispatched' => 1));
            }

            $sql = "INSERT INTO wallboard (entity_id,agent_name,dispatched_on) VALUES('".$this->id."','".$this->assigned_id."','".$current_time."')";
            $this->db->query($sql);
        ...
        if ($status == self::STATUS_POSTED) {
            ...
            $current_time = date("Y-m-d H:i:s");
            if($this->original_posted == ""){
                $updt = array(
                    'posted' => $current_time,
                    'original_posted' => $current_time,
                    'is_posted' => 1
                );
                $this->update($updt);
            } else {
            ...
            $sql = "INSERT INTO wallboard (entity_id,agent_name,posted_on) VALUES('".$this->id."','".$this->assigned_id."','".$current_time."')";
            $this->db->query($sql);

            $this->postToCentralDispatch(1);
        } else {

            $sql = "DELETE FROM wallboard WHERE entity_id =".$this->id;
            $this->db->query($sql);
    ...
    public function setStatusAndDate($status = null, $dateTime = null, $dateStatus = false)
    {
        ...
        if ($status == self::STATUS_DELIVERED) {
            if (!$this->isPaidOff()) {
                ...
                if ($delivery_confirmation_mail != "") {
                    if((!preg_match("/^([a-z0-9\+_\-]+)(\.[a-z0-9\+_\-]+)*@([a-z0-9\-]+\.)+[a-z]{2,6}$/ix", $delivery_confirmation_mail))){
                        // nothing to do
                    } else {
        ...
        if ($status == Entity::STATUS_PICKEDUP) {
            if ($dateStatus == true) {
                ...
                if($this->getAssigned()->getCompanyProfile()->dispatch_email != ""){
                    
                    $account = new Account($this->db);
                    $account->load($this->carrier_id);
                    
                    $subject = $this->prefix."-".$this->number." has been marked Picked Up by ".$_SESSION['member']['contactname'];
                    $body = "Dear ".$this->getAssigned()->contactname.",<br>User ".$_SESSION['member']['contactname']." has marked order ID ".$this->prefix."-".$this->number." as picked up by ".$account->company_name." on ".date("m-d-Y h:i A", strtotime($dateTime))."<br><br>Thank You,<br>Dispatch Department<br>".$this->getAssigned()->getCompanyProfile()->dispatch_email."<br>".$this->getAssigned()->getCompanyProfile()->dispatch_phone;
                    ...
                    $mail->AddAddress($this->getAssigned()->email);
                    $mail->setFrom($this->getAssigned()->getCompanyProfile()->dispatch_email);
                    $mail->send();
                }
    ...
    {
        if (!ctype_digit((string) $assign_id)) {
            throw new FDException("Invalid Assign ID");
        }
    ...
    {
        if (!array_key_exists($type, self::$type_name)) {
    ...
    public function split($vehicle_ids = null)
    {
        ...
        try {
            ...
            $insert_arr['source_id'] = $this->referred_id;
        }
     ...
     * @throws FDException
     */
    public function splitDispatched($vehicle_ids = null)
    {
        if (!$this->loaded) {
            throw new FDException("Entity not loaded.");
        }
        ...
        $dispatchSheet = $this->getDispatchSheet();
        $dispatchSheetNew = new DispatchSheet($this->db);
        $dispatchSheetNew->create([
            'entity_id' => $new_entity->id,
            'carrier_id' => $dispatchSheet->carrier_id,
            'account_id' => $dispatchSheet->account_id,
            'order_number' => $dispatchSheet->order_number,
            'c_companyname' => $dispatchSheet->c_companyname,
            'c_address1' => $dispatchSheet->c_address1,
            'c_address2' => $dispatchSheet->c_address2,
            'c_city' => $dispatchSheet->c_city,
            'c_state' => $dispatchSheet->c_state,
            'c_zip_code' => $dispatchSheet->c_zip_code,
            'c_phone' => $dispatchSheet->c_phone,
            'c_dispatch_contact' => $dispatchSheet->c_dispatch_contact,
            'c_dispatch_phone' => $dispatchSheet->c_dispatch_phone,
            'c_dispatch_fax' => $dispatchSheet->c_dispatch_fax,
            'c_dispatch_accounting_fax' => $dispatchSheet->c_dispatch_accounting_fax,
            'c_icc_mc_number' => $dispatchSheet->c_icc_mc_number,
            'carrier_company_name' => $dispatchSheet->carrier_company_name,
            'carrier_contact_name' => $dispatchSheet->carrier_contact_name,
            'us_dot' => $dispatchSheet->us_dot,
            'carrier_email' => $dispatchSheet->carrier_email,
            'carrier_phone_1' => $dispatchSheet->carrier_phone_1,
            'carrier_phone_2' => $dispatchSheet->carrier_phone_2,
            'carrier_phone1_ext' => $dispatchSheet->carrier_phone1_ext,
            'carrier_phone2_ext' => $dispatchSheet->carrier_phone2_ext,
            'carrier_phone_cell' => $dispatchSheet->carrier_phone_cell,
            'carrier_fax' => $dispatchSheet->carrier_fax,
            'carrier_driver_name' => $dispatchSheet->carrier_driver_name,
            'carrier_driver_phone' => $dispatchSheet->carrier_driver_phone,
            'carrier_address' => $dispatchSheet->carrier_address,
            'carrier_address_2' => $dispatchSheet->carrier_address_2,
            'carrier_city' => $dispatchSheet->carrier_city,
            'carrier_state' => $dispatchSheet->carrier_state,
            'carrier_zip' => $dispatchSheet->carrier_zip,
            'carrier_country' => $dispatchSheet->carrier_country,
            'carrier_print_name' => $dispatchSheet->carrier_print_name,
            'carrier_insurance_iccmcnumber' => $dispatchSheet->carrier_insurance_iccmcnumber,
            'carrier_type' => $dispatchSheet->carrier_type,
            'entity_load_date' => $dispatchSheet->entity_load_date,
            'entity_load_date_type' => $dispatchSheet->entity_load_date_type,
            'entity_delivery_date' => $dispatchSheet->entity_delivery_date,
            'entity_delivery_date_type' => $dispatchSheet->entity_delivery_date_type,
            'entity_ship_via' => $dispatchSheet->entity_ship_via,
            'entity_vehicles_run' => $dispatchSheet->entity_vehicles_run,
            'entity_price' => $dispatchSheet->entity_price,
            'entity_carrier_pay' => $dispatchSheet->entity_carrier_pay,
            'entity_carrier_pay_c' => $dispatchSheet->entity_carrier_pay_c,
            'entity_odtc' => $dispatchSheet->entity_odtc,
            'entity_coc' => $dispatchSheet->entity_coc,
            'entity_coc_c' => $dispatchSheet->entity_coc_c,
            'entity_booking_number' => $dispatchSheet->entity_booking_number,
            'entity_buyer_number' => $dispatchSheet->entity_buyer_number,
            'entity_pickup_terminal_fee' => $dispatchSheet->entity_pickup_terminal_fee,
            'entity_dropoff_terminal_fee' => $dispatchSheet->entity_dropoff_terminal_fee,
            'entity_balance_paid_by' => $dispatchSheet->entity_balance_paid_by,
            'from_name' => $dispatchSheet->from_name,
            'from_name2' => $dispatchSheet->from_name2,
            'from_company' => $dispatchSheet->from_company,
            'origin_auction_name' => $dispatchSheet->origin_auction_name,
            'from_booking_number' => $dispatchSheet->from_booking_number,
            'from_buyer_number' => $dispatchSheet->from_buyer_number,
            'from_address' => $dispatchSheet->from_address,
            'from_address2' => $dispatchSheet->from_address2,
            'from_city' => $dispatchSheet->from_city,
            'from_state' => $dispatchSheet->from_state,
            'from_zip' => $dispatchSheet->from_zip,
            'from_country' => $dispatchSheet->from_country,
            'from_phone_1' => $dispatchSheet->from_phone_1,
            'from_phone_2' => $dispatchSheet->from_phone_2,
            'from_phone_3' => $dispatchSheet->from_phone_3,
            'from_phone_4' => $dispatchSheet->from_phone_4,
            'from_phone1_ext' => $dispatchSheet->from_phone1_ext,
            'from_phone2_ext' => $dispatchSheet->from_phone2_ext,
            'from_phone3_ext' => $dispatchSheet->from_phone3_ext,
            'from_phone4_ext' => $dispatchSheet->from_phone4_ext,
            'from_phone_cell' => $dispatchSheet->from_phone_cell,
            'from_phone_cell2' => $dispatchSheet->from_phone_cell2,
            'from_fax' => $dispatchSheet->from_fax,
            'from_fax2' => $dispatchSheet->from_fax2,
            'to_name' => $dispatchSheet->to_name,
            'to_name2' => $dispatchSheet->to_name2,
            'to_company' => $dispatchSheet->to_company,
            'to_auction_name' => $dispatchSheet->to_auction_name,
            'to_booking_number' => $dispatchSheet->to_booking_number,
            'to_buyer_number' => $dispatchSheet->to_buyer_number,
            'to_address' => $dispatchSheet->to_address,
            'to_address2' => $dispatchSheet->to_address2,
            'to_city' => $dispatchSheet->to_city,
            'to_state' => $dispatchSheet->to_state,
            'to_zip' => $dispatchSheet->to_zip,
            'to_country' => $dispatchSheet->to_country,
            'to_phone_1' => $dispatchSheet->to_phone_1,
            'to_phone_2' => $dispatchSheet->to_phone_2,
            'to_phone_3' => $dispatchSheet->to_phone_3,
            'to_phone_4' => $dispatchSheet->to_phone_4,
            'to_phone1_ext' => $dispatchSheet->to_phone1_ext,
            'to_phone2_ext' => $dispatchSheet->to_phone2_ext,
            'to_phone3_ext' => $dispatchSheet->to_phone3_ext,
            'to_phone4_ext' => $dispatchSheet->to_phone4_ext,
            'to_phone_cell' => $dispatchSheet->to_phone_cell,
            'to_phone_cell2' => $dispatchSheet->to_phone_cell2,
            'to_fax' => $dispatchSheet->to_fax,
            'to_fax2' => $dispatchSheet->to_fax2,
            'instructions' => $dispatchSheet->instructions,
            'dispatch_terms' => $dispatchSheet->dispatch_terms,
            'created' => $dispatchSheet->created,
            'accepted' => $dispatchSheet->accepted,
            'rejected' => $dispatchSheet->rejected,
            'cancelled' => $dispatchSheet->cancelled,
            'signed' => $dispatchSheet->signed,
            'status' => $dispatchSheet->status,
            'deleted' => $dispatchSheet->deleted,
            'modified_by' => $dispatchSheet->modified_by,
            'modified_ip' => $dispatchSheet->modified_ip,
            'signature' => $dispatchSheet->signature,
            'hash_link' => $dispatchSheet->hash_link,
            'expired' => $dispatchSheet->expired,
            'payments_terms' => $dispatchSheet->payments_terms,
            'sign_by' => $dispatchSheet->sign_by,
        ]);
        foreach ($vehicle_ids as $vehicle_id) {
            $vehicle = new Vehicle($this->db);
            $vehicle->load($vehicle_id);
    ...
    public function convertToQuoteNew()
    {
        ...
        $this->setStatus(Entity::STATUS_ACTIVE);
        $prefix = $this->getNewPrefix();
        $this->update(array('quoted' => date('Y-m-d H:i:s'), 'prefix' => $prefix, 'type'=>2));
        $this->getVehicles(true);
    }
    ...
    public function convertToPendingOrder()
    {
        $prefix = $this->getNewPrefix();
    ...
    protected function sendSystemEmailNewCustomSend($type, $add = array(), $is_default = true, $emailArr)
    {
        ...
        try {
            ...
            $parsed = $this->getMailHashEmbededInBody($mail->Body);
            ...
            if ($_SESSION['member_id'] == 1 && $emailTemplate->sys_id == 7 && $type == 690) {
                foreach ($att as $name => $attachment) {
                    $attFile = ROOT_PATH . "uploads/temp/order_confirmation.pdf";
            ...
            // quotes version 1.0.2 email template
            if ($emailTemplate->sys_id == EmailTemplate::SYS_QUOTES_REUQEST_2_MAIL) {
                $attachments = [];
                foreach ($att as $name => $attachment) {
                    $filename = pathinfo($name, PATHINFO_FILENAME);
                    $attFile = ROOT_PATH . "uploads/temp/" . md5(mt_rand()) . ".pdf";
            ...
            $mail_log = array(
                ...
                'from_email' => $this->getAssigned()->getDefaultSettings()->smtp_from_email,
                'to_email' => $emailArr['to'],
                'cc_email' => json_encode($emailArr['cc']) == "null" ? "" : json_encode($emailArr['cc']),
                'bcc_email' => json_encode($emailArr['bcc']) == "" ? "" : $emailArr['bcc'],
                'email_name' => $emailTemplate->getTemplateName(),
                'subject' => $emailTemplate->getSubject(),
                'body' =>  $mail->Body
            );
            
            $this->log_mail($mail_log);
     ...
     * @return String
     */
    public function getMailHashEmbededInBody($body){
    ...
    public function getEmailContent($type, $add = array(), $is_default = true)
    {
        $tpl = new template();
        $emailTemplate = new EmailTemplate($this->db);
        $emailTemplate->setTemplateBuilder($tpl);
        ...
        $emailContentArr['to'] = $emailTemplate->getToAddress();
        $emailContentArr['cc'] = "";
        $emailContentArr['bcc'] = $emailTemplate->getBCCs();
        $emailContentArr['body'] = $emailTemplate->getBody();
        ...
        $emailContentArr['name'] = $emailTemplate->name;
        return $emailContentArr;
    }
    ...
    public function sendOrderDispatched()
    {
        ...
        $this->sendSystemMail(EmailTemplate::SYS_ORDER_DISP_NOTIFY);
    }

    // $is_default = true for default system templates
    // set to false for needed template id
    protected function sendSystemMail($type, $add = array(), $is_default = true)
    {
        $tpl = new template();
        $emailTemplate = new EmailTemplate($this->db);
        $emailTemplate->setTemplateBuilder($tpl);
    ...
    public function sendOrderDispatchedGeneric()
    {
    ...
    public function sendDispatchLink($add)
    {
        ...
        if(getParentId() == Member::PARENT_CARGOFLARE_ID){
            $this->sendSystemEmail(EmailTemplate::SYS_ORDER_DISPATCH_LINK_CARGOFLARE, $add);
        } else {
    ...
    public function sendAQTemplate($template_id)
    {
        $tpl = new template();
        $emailTemplate = new EmailTemplate($this->db);
        $emailTemplate->setTemplateBuilder($tpl);
        $emailTemplate->loadTemplate_v2($template_id, $this->getAssigned()->parent_id, $this, [], false);
        ...
        try {
            ...
            if ($emailTemplate->sys_id == EmailTemplate::SYS_ORDER_DISP_SHEET_ATT) {
                $path = ROOT_PATH . "uploads/temp/" . md5(mt_rand()) . ".pdf";
                $this->getDispatchSheet()->getPdfNew("F", $path);
                $attachments["Attachment.pdf"] = $path;
            }
            foreach ($attachments as $name => $attachment) {
                $mail->AddAttachment($attachment, $name);
            }
            ...
            $mail_log = array(
                ...
                'hash' => $hash,
                'from_email' => $this->getAssigned()->getDefaultSettings()->smtp_from_email,
                'to_email' => $emailTemplate->getToAddress(),
                'cc_email' => "",
                'bcc_email' => json_encode($emailTemplate->getBCCs())."",
                'email_name' => $emailTemplate->getTemplateName(),
                'subject' => $emailTemplate->getSubject(),
                'body' =>  $mail->Body
            );
    ...
    public function log_mail($data){
        $this->db->insert('entity_email_log', $data);
    }
    ...
    public function state2Id($state)
    {
        return $state;
    }
    ...
    public function getVehicleType($vehicleTypeValue)
    {
    ...
    public function check_match_carrier($entity_old)
    {
    ...
    public function checkVehicleChange($entity_old)
    {
     ...
     */
    public function sendMatchCarrierEmail($toEmail, $emailBody, $subject)
    {
     ...
     * @throws FDException
     */
    public function sendEmailMatchCarrierUsingLocalSetting($type, $add = array(), $is_default = true, $toEmail = '', $accountIDMatchCarrier = 0)
    {
     ...
     * @version 1.0
     */
    public function sendMatchCarrierCustomSMTP($type, $add = array(), $is_default = true, $toEmail = '', $accountIDMatchCarrier = 0)
    {
        ...
        try {
            ...
            $mail->SMTPAuth = true;
            // smptp username here
            $mail->Username = 'match@ritewayautotransport.com';
            // smtp password here
            $mail->Password = 'f37c51fc5cd76f53984a79b3d0dfc025-2dfb0afe-951b91ec';
            //smtp port here
            $mail->Port = '587';
            $mail->SMTPSecure = 'tls';
    ...
    public function sendEmailMatchCarrier($type, $add = array(), $is_default = true, $toEmail = '', $accountIDMatchCarrier = 0)
    {
