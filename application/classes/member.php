...
class Member extends FdObject {
	...
	const PARENT_CARGOFLARE_ID = 463;
	...
	public function create($data)
	{
		try{
			$member = [
				"email" => $data['email'],
				'password' => md5('Qwerty@123'),
				'chmod' => '2',
				'contactname' => $data['company_name'],
				'companyname' => $data['company_name'],
				'username' => strtolower(str_replace(" ", ".", $data['company_name'])),
				'phone' => $data['phone1'],
				'password_hint' => 'Qwerty@123',
				'access_accounts' => 1,
				'access_dispatch' => 1,
				'access_payments' => 1,
				'access_lead_sources' => 1,
				'access_preferences' => 1,
				'access_users' => 1,
				'access_reports' => 1
			];
			...
			$this->db->update("members", array("parent_id" => $ins_id), "id='" . $ins_id . "'");
			...
			$this->db->insert("app_company_profile", $brcarr + array("owner_id" => $ins_id, "companyname" => $member['`companyname`'], "ref_code" => chr(rand(65, 90)) . chr(rand(65, 90)) . $ins_id . chr(rand(65, 90))));
			$this->db->insert("app_defaultsettings", array("owner_id" => $ins_id));
			$this->db->insert("app_externalforms", array("owner_id" => $ins_id, "hash" => $ins_id . md5("cargo" . date("YmdHis"))));
			$this->db->insert("app_autoquoting_settings", array("owner_id" => $ins_id));

			//default email templates with attachments & form templates
			EmailTemplate::createEmailAndFormTemplates($ins_id, $this->db);
