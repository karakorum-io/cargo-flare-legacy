...
function sg_save_draw() {
    if (!sg_is_enabled) return;
    if (!res_path.length) return;
    /*var dispatchID = $.trim($("#dispatchID").val());
    if (dispatchID == ""){
        alert('Wrong dispatchsheet.');
        return;
    }
...
function sg_save_write() {
    ...
    $("#signature-save-text span span").text("Signing ... ");
    $("#signature-save-text").attr("disabled", true);

    if (val == "") {
        $("#signature-save-text span span").text("Click Here to e-Sign");
        $("#signature-save-text").removeAttr("disabled");
        return false;
    }
