...
function displayNotesNew(notes_type, notes_data) {
	...
	for (i in notes_data) {
		...
		container += "<\/span><br/><span class='note-data' style='color:"+bgcolor+";"+discardStr+"'>";
...
function selectTerminal(location) {
    ...
    $("#acc_search_string").val("");
    acc_type = 3;
    $("#acc_search_dialog").dialog({width: 650},{title:'Select Terminal'}).dialog('open');
}
    ...
    $.ajax({
        ...
        success: function(response){
            if (response.success) {
				...
				AccRows += '<td '+bgColor+'>Company</td>';
				        
						if(acc_type==3) {
					         AccRows +='<td '+bgColor+'>Contact Name</td><td '+bgColor+'>Address</td><td '+bgColor+'>Phone</td><td '+bgColor+'>City</td>';
						 }
						else
						 {
				           AccRows +='<td '+bgColor+'>Address</td><td '+bgColor+'>Phone</td><td '+bgColor+'>Email</td>';
						 }
					...
					if(acc_type==2) 
				     {
						...
						AccRows += '</td><td '+bgColorRows+'>'; 
                        if(acc_type==3) {
					         AccRows += acc_data[i].contact_name1;
						 }  
						AccRows += '</td><td '+bgColorRows+'>';  
    ...
    $.ajax({
        ...
        success: function(response){
            if (response.success) {
				...
				AccRows +='<td '+bgColor+'>State</td><td '+bgColor+'>Insurance Type</td><td '+bgColor+'>Insurance</td><td '+bgColor+'>Expire</td></tr>';
					...
					for (i in acc_data) {
					  ...
					  if(acc_data[i].rowcolor !='')
						...
						AccRows += '<tr><td '+bgColorRows+' '+bgColor+'><input type="radio" name="acc_search_result_item" class="form-box-radio" id="acc_search_result_item'+i+'" value="'+i+'"/></td><td '+bgColorRows+' '+bgColor+'>';
...
function repostOrderFromFB(entity_id) {
	...
	$.ajax({
		...
		success: function(response) {
			if (response.success) {
				alert("RePosted order to Freight Board.");
			} else {
			...
			success: function (res) {
				if(res.success=="single"){
					...
					for (j = 1; j <= countEmails; j++) {
						...
						mailcontent += '<div class="edit-mail-container" id="edit-mail-show_'+j+'"><div style="float: left;"><ul><li style="margin-bottom: 16px;padding-top: 5px; color: forestgreen;font-weight: bold">Sending Options</li><li style="margin-bottom: 14px;">Form Type <input value="1" id="attachPdf_'+j+'" '+attachPdf+' name="attachT'+j+'" type="radio"/><label for="attachPdf_'+j+'" style="margin-right: 2px; cursor:pointer";> PDF</label><input value="0" id="attachHtml_'+j+'" '+attachHtml+' name="attachT'+j+'" type="radio"/><label for="attachHtml_'+j+'" style="cursor:pointer"> HTML</label></li><li style="margin-bottom: 11px;">Attachment(s):<span style="color:#24709F;"> '+mail_att+'</span></li></ul></div><div style="text-align: right;"><ul><li style="margin-bottom: 6px;"><span>Email:<span style="color:red">*</span></span> <input type="text" id="mail_to_'+j+'" name="mail_to" value="'+mail_to+'" class="form-box-combobox" ></li><li style="margin-bottom: 6px;"><span style="margin-right: 18px;">CC:</span> <input type="text" id="mail_cc_'+j+'" name="mail_cc" value="'+mail_cc+'" class="form-box-combobox" ></li><li style="margin-bottom: 12px;"><span style="margin-right: 9px;">BCC:</span> <input type="text" id="mail_bcc_'+j+'" name="mail_bcc" value="'+mail_bcc+'" class="form-box-combobox" ></li></ul></div><div class="edit-mail-content" style="margin-bottom: 8px;"><div class="edit-mail-row" style="margin-bottom: 8px;"><div class="edit-mail-label">Subject:<span>*</span></div><div class="edit-mail-field" style="width: 87%;"><input type="text" id="mail_subject_'+j+'" class="form-box-textfield" maxlength="255" name="mail_subject" style="width: 100%;" value="'+mail_subject+'"></div></div><div class="edit-mail-row"><div class="edit-mail-label">Body:<span>*</span></div><div class="edit-mail-field" style="width: 87%;"><textarea class="form-box-textfield" style="width: 100%;" name="mail_body" id="mail_body_'+j+'">'+mail_body+'</textarea></div></div></div><div class="edit-mail-button"><div class="edit-mail-action update">Update</div><div class="edit-mail-action cancel">Cancel</div></div></div>';
					}
					$(".editmail").empty();
					...
					for (i = 1; i <= countEmails; i++) {
						...
						content += '<tr class="'+trclass+'"><td class="order-id id-column" style="border-left:1px solid #d3d3d5;width:70px">'+oids[lesscount]+'</td><td class="shipper-column" style="width: 229px;"><div>'+shipper_name[lesscount]+'<div><div>'+shipper_company[lesscount]+'<div><div style=" word-wrap: break-word;">'+shipper_number[lesscount]+'<div><div>'+shipper_email[lesscount]+'<div></td><td style="padding: 5px 10px;text-align: center;width: 90px;">'+mail_att+'</td><td class="action-column" style="width: 29px;"><div class="go-to-edit" id="mail-edit-link_'+i+'">Edit</div></td></tr>';
					}
					...
					$(".repeat-column").show();
					$("#listmails").dialog({width: 566,height:340}).dialog("open");
...
$('.send-mail').live('click' ,function(){
        var optionemail="";optioncc="";optionbcc = ""; 
        var combine= 0;
...
function emailSelectedLeadFormNew(nameObj) {
	...
	if ($("."+nameObj+"-checkbox:checked").size() == 0) {
		$(".alert-message").empty();
		$(".alert-message").text("Order not selected.");
		$(".alert-pack").show();
		return false; 
		...
		$.ajax({
			...
			success: function (res) {
				if(res.success=="single"){
					...
					content += '</tbody></table>';
					//content += '<div class="list-container"><div class="email-counter">Email </div><div class="go-to-edit" id="mail-edit-link_'+i+'">Edit</div></div>';
					content += '<div class="edit-mail-container2"><div class="edit-mail-button"><div class="edit-mail-action send-mail-lead"><input type="hidden" id="count-total-mail" value="'+countEmails+'">Send Mail</div></div></div>';
...
$('.send-mail-lead').live('click' ,function(){
	 var optionemail="";optioncc="";optionbcc = ""; 
        var combine= 0;
        if ($('.optionemail').val().length > 0){
            var sEmail = $('.optionemail').val();
