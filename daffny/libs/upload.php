...
class upload
{
    ...
    */
    ...
    function upload_process()
    {
        ...
        //-------------------------------------------------
        if ($_FILES[$this->form_field]['name'] == ""
        or !$_FILES[$this->form_field]['name']
        or !$_FILES[$this->form_field]['size']
        or ($_FILES[$this->form_field]['name'] == "none") )
        {
            $this->error_no = 1;
            $this->error_str = "No upload";
        ...
        {
            $this->error_no = 2;
            $this->error_str = "Not valid upload type";
        ...
        {
            $this->error_no = 3;
            $this->error_str = "Upload exceeds ".$this->max_file_size;
            return;
        }
        ...
        //-------------------------------------------------
        $this->parsed_file_name = $this->_unique_filename($this->parsed_file_name);
        
        if ($this->save_as_file_name == "") {
            $this->saved_upload_name = $this->out_file_dir.'/'.$this->parsed_file_name;
        }
        else {
            $this->saved_upload_name = $this->out_file_dir.'/'.$this->save_as_file_name;
        }
        
        if ( ! @move_uploaded_file($_FILES[ $this->form_field ]['tmp_name'], $this->saved_upload_name) )
        {
            $this->error_no = 4;
            $this->error_str = "Could not move uploaded file, upload deleted";
            return;
        }
        else
        {
        ...
        if ( $this->is_image )
        {
            ...
            if ( $this->image_check )
            {
                ...
                if ( ! is_array( $img_attributes ) or ! count( $img_attributes ) )
                {
                    ...
                    $this->error_no = 5;
                    $this->error_str = "File pretending to be an image but isn't";
                    return;
                }
    ...
    /*------------------------------------*/
    // INTERNAL: Get Unique FileName
    /*------------------------------------*/
    function _unique_filename($filename)
    {
        if ( file_exists($this->out_file_dir."/".$filename) )
        {
            $file = str_replace(".".$this->file_extension, "", $filename);
            
            for ( $i=0; $i<999; $i++ )
            {
                $tmpname = $file."($i).".$this->file_extension;
                
                if ( !file_exists($this->out_file_dir."/".$tmpname) )
                {
