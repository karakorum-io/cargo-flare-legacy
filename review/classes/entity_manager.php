...
//$row = $this->db->selectRow("COUNT(e.`id`) as followup", "app_entities as e Inner join app_followups as f", "WHERE e.`deleted` = 0 AND e.`type` = " . $type . " AND " . $this->getPermissionCondition($type, false,"e") . " ");  
 ...
 public function getCountCreatedLead($type,$extra=0)
  ...
  public function getCountHeader($type)
  {
      ...
      // $result[Entity::STATUS_ARCHIVED] = $archivedTotal;
	   //$row = $this->db->selectRow("COUNT(`id`) as followup", self::TABLE, "WHERE `deleted` = 0 AND `type` = " . $type . " AND " . $this->getPermissionCondition($type, false) . " AND `id` IN (SELECT `entity_id` FROM " . FollowUp::TABLE . " )");  //WHERE `followup` = '" . date("Y-m-d") . "'
//print "WHERE `deleted` = 0 AND `type` = 2 AND " . $this->getPermissionCondition($type, false) . " AND `id` IN (SELECT `entity_id` FROM " . FollowUp::TABLE . " )";
       //$result[0] = $row['followup'];
/*	   
	   $sql = "select COUNT(e.`id`) as followup
FROM app_entities as e 
...
*/
    ...
    {
        //$where = "WHERE `deleted` = 0 AND type='".Entity::TYPE_QUOTE."' AND " . $this->getPermissionCondition(Entity::TYPE_QUOTE, false) . " AND `id` IN (SELECT `entity_id` FROM `app_followups` )";  // WHERE `followup` = '" . date("Y-m-d") . "'
    ...
    if (!is_null($per_page) && ctype_digit((string)$per_page)) {
			...
			$this->pager->UrlStart = getLink();
			$this->pager->RecordsOnPage = $per_page;
			$this->pager->init(' app_entities as e 
Inner join app_followups as f
ON e.id = f.entity_id '.$where_str, "", trim());
			$where_str_limit = " ".$this->pager->getLimit()." ";
		}
...
/*
$sql = "SELECT 		CASE isnull (A.date_received) 
			...
			B.PayIn,
			A.PayOut
FROM (SELECT 	DATE_FORMAT(date_received,'%Y-%m-%d') date_received, 
				DATE_FORMAT(date_received,'%a') date_received_day, 
				CASE fromid 
					WHEN 1 THEN 'COMPANY' 
					WHEN 2 THEN 'SHIPPER' 
					WHEN 3 THEN 'CARRIER' 
		...
		FROM  	`app_payments` 
		WHERE  	DATE_FORMAT(date_received ,'%Y-%m-%d') >= DATE_FORMAT('" . $start_date . "' ,'%Y-%m-%d')  AND DATE_FORMAT(date_received ,'%Y-%m-%d') <= DATE_FORMAT('" . $end_date . "' ,'%Y-%m-%d') 
		and deleted = 0
		and (fromid = 1 and toid = 3)
		group by DATE_FORMAT(date_received,'%m-%d-%Y'),  
				DATE_FORMAT(date_received,'%a'), 
				CASE fromid 
					WHEN 1 THEN 'COMPANY' 
					WHEN 2 THEN 'SHIPPER' 
					WHEN 3 THEN 'CARRIER' 
				...
				(SELECT 	DATE_FORMAT(date_received,'%Y-%m-%d') date_received, 
					DATE_FORMAT(date_received,'%a') date_received_day, 
					CASE fromid 
						WHEN 1 THEN 'COMPANY' 
						WHEN 2 THEN 'SHIPPER' 
...
UNION
		SELECT 		CASE isnull (A.date_received) 
				When 1 then B.date_received
				else A.date_received end  date_received,
			CASE isnull (A.date_received_day) 
				When 1 then B.date_received_day
...
public function searchAll($type, $search_type, $value, $per_page = 50, $order = null)
 {
		...
		*/
		...
		if (!is_null($order)) $where_s .= " ".$order." ";
...
public function getArrDataHeader($per_page = 50,$tables, $where = null, $order = null)  {
       ...
       $where_str_limit = "";
        ...
        if (!is_null($order)) $where_str .= " ".$order." ";
		//if (!is_null($order)) 
...
//print_r($search_type);
					...
					$where = " e.`deleted` = 0 ".$mtype." ".$whereCancel."  ";							
					$whereTemp = "";
					foreach($search_type as $valueType)
					{
...
//print_r($search_type);
      ...
      //$StatusQuery = "WHERE ".$where." AND e.`type` = {$type} AND " . $this->getPermissionCondition($type, false, "e");
	   
      //$TypeQuery = "WHERE ".$where."  AND " . $this->getPermissionCondition($type, false, "e");
	  ...
	  $where .= " AND e.`type` = {$type} AND (".$this->getPermissionCondition($type, false, 'e')."  OR e.`salesrepid` ='".$_SESSION['member_id'] . "')";
	   ...
	   $rows = $this->db->selectRows("e.status as status,count(*) as cnt", $tables, $where_str);
	   //$rows = $this->db->selectRows("e.status as status,count(*) as cnt", $tables, $StatusQuery);
...
function split_vehicle_search($searchStr)
	{
		  $year   = date('Y');
		  $vYear  = '';
		  $vMake  = '';
