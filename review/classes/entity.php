...
/* UPDATE NOTE */
    ...
    public function combineattchformdb($emailArr,$member_id){  
	...
	$member->load($member_id);
      try {
           ...
           $ext= pathinfo($extsql['name'], PATHINFO_EXTENSION);
            ...
            $i=1;  
            foreach ($rows as $row){
            $filename = pathinfo($row['name'], PATHINFO_FILENAME);
            
                $mail->AddAttachment($row['attachment_path'], $filename.$i++.$ext);
            }  
