	...
	***************************************************************************************************/
    ...
    class AutoQuotingLane extends FdObject {
        ...
        public function duplicate($seasonId = null) {
            if (!ctype_digit((string)$seasonId)) throw new FDException("Invalid Season ID");
            try {
                if (is_null($seasonId)) $this->db->transaction("start");
                ...
                if (is_null($seasonId)) $this->db->transaction("commit");
            } catch (FDException $e) {
                if (is_null($seasonId)) $this->db->transaction("rollback");
                throw $e;
            }
        }
        ...
        public function updateVehicleSurcharges($data) {
            if (!is_array($data)) throw new FDException("Invalid input data");
            if (count($data)>0){
